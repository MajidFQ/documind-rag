...
**PDF table extraction flattens structure.** `pypdf` extracts table content as plain text, losing row/column alignment. A course name and its corresponding grade may both appear in the extracted text but are no longer clearly associated — the visual structure that made the mapping obvious is gone. A more robust approach would use table-aware extraction (e.g. `pdfplumber` or `camelot`) to detect tables and convert them to structured markdown before chunking, preserving row/column relationships.

**Conversation history is not persisted.** Chat history in the Streamlit UI lives only in `st.session_state` — in-memory for the current browser session. Refreshing the page or restarting the app clears the conversation, even though the underlying documents and ChromaDB embeddings persist across restarts. A future improvement would store Q&A history in a lightweight database (e.g. SQLite) keyed by session or user, so past conversations survive page reloads.
